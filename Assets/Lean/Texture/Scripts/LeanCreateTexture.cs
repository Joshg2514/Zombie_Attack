...
namespace Lean.Texture
{
	...
	public class LeanCreateTexture : LeanCreate
	{
		...
		{
			LeanEditor.BeginError(ObjectExists("texture") == false);
				LeanEditor.Draw("texture", "The texture that will be used to create the texture.");
			LeanEditor.EndError();
			LeanEditor.Draw("overrideSize", "The size of the created texture.\n\n-1 = Use existing texture size.");
		}
#endif
	}
}