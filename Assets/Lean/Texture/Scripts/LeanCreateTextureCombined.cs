...
namespace Lean.Texture
{
	...
	public class LeanCreateTextureCombined : LeanCreate
	{
		...
		{
			LeanEditor.Draw("overrideSize", "By default the created texture will be given the largest width and largest height of the input textures, but this setting allows you to override this.\n\n-1 = Do not override size.");

			LeanEditor.Separator();

			LeanEditor.BeginError(ObjectExists("redTexture") == false && ObjectExists("redTexture") == false && ObjectExists("redTexture") == false && ObjectExists("redTexture") == false);
		...
		private void DrawChannel(string title, string pathA, string pathB)
		{
			UnityEditor.EditorGUILayout.BeginHorizontal();
				UnityEditor.EditorGUILayout.PropertyField(LeanEditor.GetProperty(pathA), new GUIContent(title));
				UnityEditor.EditorGUILayout.PropertyField(LeanEditor.GetProperty(pathB), GUIContent.none, GUILayout.Width(80));
